  ...
  <div class="container-fluid">
      <div class="row clearfix">
          <div class="col-lg-12 col-md-12 col-sm-12">
              <div class="card">
                  <div class="header">
                    <a href="{{ route('specification.create') }}" class="btn  btn-sm btn-icon waves-effect waves-light btn-success float-right"
                      ...
                      <div class="table-responsive">
                          <table class="table table-bordered table-condensed table-hover js-basic-example dataTable">
                              ...
                              <tbody>
                                ...
                                <tr>
                                     {{-- <td>{{ $value->name }}</td> --}}
                                     <td>{{Str::limit($value->spec_name, 10, $end='.....')}}</td>
                                     <td>{{ $value->product->product_name }}</td>
                                     <td>{{ $value->spec_icon }}</td>
                                     <td>{{Str::limit($value->description, 10, $end='.....')}}</td>
                                     <td>{{Str::limit($value->remark, 10, $end='.....')}}</td>
                                     ...
                                     {{-- <td>{{ $value->remarks }}</td> --}}
                                        @if($value->status == "active")
                                          ...
                                          @endif
                                    <td>
                                        <form action="{{ route('specification.destroy',$value->id) }}" method="POST">       
                                            <a class="btn  btn-sm btn-icon  btn-info"  href="{{ route('specification.edit',$value->id) }}" ><i class="fa fa-edit"></i>
                                            </i></a>
                                            @csrf
                                            @method('DELETE')
